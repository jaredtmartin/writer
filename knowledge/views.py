...
def knowledge_list(request,
    ...
    articles = Article.objects.can_view(request.user).prefetch_related('question__responses__question')
    ...
    paginator, articles = paginate(articles,
                                    50,
...
def knowledge_thread(request,
    ...
    try:
        article = Article.objects.can_view(request.user).get(id=article_id)
    except Article.DoesNotExist:
        if Article.objects.filter(id=article_id).exists() and hasattr(settings, 'LOGIN_REDIRECT_URL'):
            return redirect(settings.LOGIN_REDIRECT_URL)
        else:
            raise Http404
    if request.path != article.get_absolute_url():
        return redirect(article.get_absolute_url(), permanent=True)
        
    try: 
        responses = article.question.get_responses(request.user)
        if request.method == 'POST':
            form = Form(request.user, article, request.POST)
            if form and form.is_valid():
                if request.user.is_authenticated() or not form.cleaned_data['phone_number']:
                    form.save()
                return redirect(article.get_absolute_url())
        else:
            form = Form(request.user, article.question)
