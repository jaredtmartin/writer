AUTHORS.txt
LICENSE
MANIFEST.in
README.rst
django_filter.egg-info/PKG-INFO
django_filter.egg-info/SOURCES.txt
django_filter.egg-info/dependency_links.txt
django_filter.egg-info/not-zip-safe
django_filter.egg-info/top_level.txt
django_filters/__init__.py
django_filters/fields.py
django_filters/filters.py
django_filters/filterset.py
django_filters/models.py
django_filters/views.py
django_filters/widgets.py
django_filters/tests/__init__.py
django_filters/tests/models.py
django_filters/tests/test_urls.py
django_filters/tests/tests.py
django_filters/tests/fixtures/test_data.json
django_filters/tests/fixtures/test_data_django_13.json
django_filters/tests/templates/tests/book_filter.html
docs/Makefile
docs/conf.py
docs/index.txt
docs/install.txt
docs/make.bat
docs/tests.txt
docs/usage.txt
docs/ref/filters.txt
docs/ref/widgets.txt